...
emsemble_volume2 <- function(ensemble_members=10000,
  ...
  for (i_year in seq_along(RMSE_LOOCV)) {
    vol.ens[,i_year] <- rnorm(ensemble_members,mean=predicted_volumes[i_year],sd=RMSE_LOOCV[[i_year]][1]) %>%
...
linear_regression <- function(data_train_predict) {
  tag="lineal"
  ...
  model <- train(volumen~ .,#poly(CI_last_v1,2),
                 ...
                 method     = "lm",
                 trControl  = trainControl(method = "cv",number = 38,savePredictions = T))
