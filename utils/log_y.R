...
source("base/Preprocess_data_v2.R")
...
cod_cuencas = read.csv(catchments_attributes_filename)$cod_cuenca [-c(13,14,29,32,40,45,49)]
...
p_values_preproceso = results %>%
  ...
  mutate(step_status = case_when(
    ...
    normal_step1 == TRUE & normal_step2 == FALSE ~ "Normal sólo en paso 1",
    normal_step1 == FALSE & normal_step2 == TRUE ~ "Normal sólo en paso 2",
    normal_step1 == FALSE & normal_step2 == FALSE ~ "Nunca normal"
  )) %>%
  mutate(month = as.factor(month_initialisation))


levels(p_values_preproceso$month) = paste0("1˚ ", months_year[as.numeric(levels(p_values_preproceso$month) )])
...
ggplot(data = p_values_preproceso,
  ...
  scale_fill_manual(values = c("Normal en ambos pasos" = "#78B7C5",
                               "Normal sólo en paso 1" = "#3B9AB2",
                               "Normal sólo en paso 2" = "#E1AF00",
                               "Nunca normal" = "#F21A00")) +
  theme_minimal() +
