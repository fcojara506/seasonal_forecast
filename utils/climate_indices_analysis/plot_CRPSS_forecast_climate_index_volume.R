...
rm(list = ls())
filename <- "data_output/scores/RDS/model_results_2023-01-20.RDS"
...
cor_matrix <- dcast(df, catchment_month ~ predictor, value.var = metric_variable, fill = NA)
...
png(paste0("test_heatmap_nr.png"), width = 10, height = 5, units = "in", res = 800)
draw(p)
dev.off()
