...
data_best_models = lapply(cod_cuencas, function(x)
  readRDS(file = paste0("data_output/mejores_modelos_cuenca_mes/",x,"_may-sep.RDS"))$importance) %>% 
  rbindlist()
...
dataframe = data_best_models %>%
  ...
  melt(id.vars = c("date_label","catchment_code"),
       variable.name  = "var",
...
#plot <- 
ggplot() +
  geom_sf(data = merged_data, aes(fill = percentage),linewidth=0.1) +
  scale_fill_continuous(low = "white", high = "red") + # Change the colors according to your preference
