...
for (Cuenca in Cuencas){
  ...
  for (Mes in Meses){
    for(AgnoLO in AgnosLO[1]){  # L3O (ahora es redundante)
      ...
      for (Loc in Locs){
        ## TRANSFORMA PRECIP
        ref      <- cr2pr[[Mes]] %>% melt(id = "date") %>%
          filter(variable %in% Loc & format(date, "%Y") %in% as.character(Agnos[! Agnos %in% AgnoLO])) %$% value %>% sqrt
        cal      <- era5pr[[Mes]] %>% melt(id = "date") %>%
          filter(variable %in% Loc & format(date, "%Y") %in% as.character(Agnos[! Agnos %in% AgnoLO])) %$% value %>% sqrt
        val      <- era5pr_val[[Mes]] %>% melt(id = "date") %>%
          filter(variable %in% Loc) %$% value %>% sqrt
        ...
        ## TRANSFORMA TEMP
        ref      <- cr2tm[[Mes]] %>% melt(id = "date") %>%
          filter(variable %in% Loc & format(date, "%Y") %in% as.character(Agnos[! Agnos %in% AgnoLO])) %$% value
        cal      <- era5tm[[Mes]] %>% melt(id = "date") %>%
          filter(variable %in% Loc & format(date, "%Y") %in% as.character(Agnos[! Agnos %in% AgnoLO])) %$% value
        val      <- era5tm_val[[Mes]] %>% melt(id = "date") %>%
      ...
      ## EXTRAE DIAS
      t.cal <- cr2pr[[Mes]] %>% dplyr::select(date) %>%
