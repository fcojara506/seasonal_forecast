...
descargar_era5 <- function(CDS_user = NULL, dias_previos_con_datos = 6) {
  ...
  t_near <- Sys.time() %>%
    ...
    as.POSIXct(tz = "GMT") %>%
    `-`(dias_previos_con_datos * 24 * 3600) %>%
    trunc(units = "hour") %>%
    as.POSIXct(tz = "GMT")
  ...
  df_t <- data.frame(date = seq(t_i, t_f, by = "day")) %>%
    mutate(mes = format(date, "%m"), agno = format(date, "%Y")) %>%
    group_by(mes) %>%
    mutate(days_month = days_in_month(date), n = n())
  # meses completos
  Meses_Agnos <- df_t %>%
    group_by(mes) %>%
    filter(days_month == n) %>%
  ...
  # mes incompleto
  Mes_Inc <- df_t %>%
    filter(days_month != n) %$% mes %>%
    unique()
  # agno incompleto
  Agno_Inc <- df_t %>%
    filter(days_month != n) %$% agno %>%
    unique()
