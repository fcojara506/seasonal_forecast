...
predictant_generator <- function(
  ...
  if (log_transform) {
    ...
    if (plot_combined) {
      plot_combined_distribution(y_converted$volume, volume_transformed, "Predictand")
    }
...
preprocess_data <- function(
    ...
    remove_wys = NULL,
    save_raw = F,
    y_transform = list(log_transform = T, plot_combined = F)
) {
  ...
  predictant <-
    predictant_generator(
      ...
      plot_combined = y_transform$plot_combined
    )
  
  info_list = append(info_list,within(predictant, rm(q,y)))
...
test_preprocess <- function(){
  ...
  data_location_paths = get_default_datasets_path(meteo = "ens30avg", hydro = "ERA5Ens_SKGE")
  save_raw = F
  y_transform = list(log_transform = F, plot_combined = F)
