...
for (Mes in Meses){
  ...
  era5pr_val[[Mes]] <- exact_extract(rast(namef), cuencas, "mean") %>% t %>%
    structure(dimnames = list(NULL, cuencas$gauge_id)) %>% cbind.data.frame(date = t_d, .) %>% filter(date >= t_i & date <= t_f)
  nc_close(nc)
  ## ERA5 temp
  namef <- paste0("data_input/preproceso_meteo/download_meteo/ERA5_temp_2023-presente_daily_m", Mes, ".nc")
  nc    <- nc_open(namef)
  t_d   <- as.Date(as.POSIXct("1900-01-01 00:00:00", tz = "UTC") + 3600 * ncvar_get(nc, "time"))
  era5tm_val[[Mes]] <- exact_extract(rast(namef), cuencas, "mean") %>% t %>%
    structure(dimnames = list(NULL, cuencas$gauge_id)) %>% cbind.data.frame(date = t_d, .) %>% filter(date >= t_i & date <= t_f)
  nc_close(nc)
}
rm(Mes, namef, nc, t_d)
