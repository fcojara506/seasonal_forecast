...
select_predictor <- function(
  ...
  models <- foreach(month_initialisation = months_initialisation,
    ...
    # keep only un correlated predictores
    ...
    #initialisation list to save results
    ...
    foreach(predictor = predictors_comb) %dopar% {
      ...
      # compute relative importance of predictors
  ...
  if (save) {
    saveRDS(best_list,file = paste0("data_input/mejores_predictores_cuenca_mes/",catchment_code,'_may-mar.RDS') )
  }
  return(best_list)
}
...
select_best_predictors <- function(objective_metric = "aic",update_predictors = F) {
  
attributes_catchments_file <- "data_input/attributes/attributes_45catchments_ChileCentral.csv"
cod_cuencas <- fread(attributes_catchments_file)$cod_cuenca

  for (catchment_code in cod_cuencas) {
    
    print(paste('Probando combinación de predictores. Cuenca:', catchment_code))
    
    select_predictor(catchment_code = catchment_code,
                     objective_metric = objective_metric,
                     save = update_predictors)

  }
}


#select_best_predictors(objective_metric = "aic",update_predictors = F)
